...
export default function MobileMenu() {
  ...
  return (
    <div className="w-32 text-right sm:hidden block">
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="inline-flex w-full justify-center rounded-md bg-black/20 px-4 py-2 text-sm font-medium text-white hover:bg-black/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75">
            Options
            <FaChevronDown
              className="-mr-1 ml-2 my-auto text-violet-200 hover:text-violet-100"
              aria-hidden="true"
            />
          </Menu.Button>
        </div>
        ...
        >
          <Menu.Items className="absolute right-0 mt-2 w-40 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none z-[10]">
            <div className="px-1 py-1 ">
              ...
              <Menu.Item>
                {
                  currentUser ? (<Link to={"/profile"}><img src={currentUser?.avatar} alt='profile' className='rounded-lg ml-2 mb-1 h-7 w-7 object-cover' referrerPolicy="no-referrer" /></Link>)
                    :
                    (
                      <button
